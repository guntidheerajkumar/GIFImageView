...
- (instancetype)init
{
    ...
    if (!animatedImage) {
        FLLog(FLLogLevelError, @"Use `-initWithAnimatedGIFData:` and supply the animated GIF data as an argument to initialize an object of type `AnimatedImage`.");
    }
                ...
                if (frameImageRef) {
                    ...
                    if (frameImage) {
                        ...
                        if ([delayTime floatValue] < ((float)kAnimatedImageDelayTimeIntervalMinimum - FLT_EPSILON)) {
                            FLLog(FLLogLevelInfo, @"Rounding frame %zu's `delayTime` from %f up to default %f (minimum supported: %f).", i, [delayTime floatValue], kDelayTimeIntervalDefault, kAnimatedImageDelayTimeIntervalMinimum);
                            delayTime = @(kDelayTimeIntervalDefault);
                        }
...
+ (CGSize)sizeForImage:(id)image
{
    ...
    if ([image isKindOfClass:[UIImage class]]) {
        ...
        FLLog(FLLogLevelError, @"`image` isn't of expected types `UIImage` or `AnimatedImage`: %@", image);
    }
...
- (void)didReceiveMemoryWarning:(NSNotification *)notification
{
    ...
    FLLog(FLLogLevelDebug, @"Attempt setting frame cache size max to %lu (previous was %lu) after memory warning #%lu for animated image: %@", (unsigned long)AnimatedImageFrameCacheSizeLowMemory, (unsigned long)self.frameCacheSizeMaxInternal, (unsigned long)self.memoryWarningCount, self);
